...
the field `arg`, an array of byte strings. The `lsig` structure is defined in the
[Authorization and Signatures](../ledger/ledger-txn-authorization.md) section of
the Ledger specifications.
...
Logic Signature arguments are _not_ signed, they are not covered by any delegation
signature and are recorded on the blockchain, publicly visible as soon as the transaction
is submitted to the network, even before it has been included in a block. These
arguments are _not_ part of the transaction ID nor of the [transaction group](../ledger/ledger-txn-groups.md)
hash (`grp`). They also cannot be read from other programs in the group of transactions.
...
account or be wholly in charge of a _contract account_.

### Delegated Signature Mode

In this mode, the account signs the program by providing exactly one of the following:

- A signature (field `sig`), for a normal signature-based account;
